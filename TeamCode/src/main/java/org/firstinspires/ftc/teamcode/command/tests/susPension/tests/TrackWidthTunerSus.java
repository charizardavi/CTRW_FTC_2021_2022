...
public class TrackWidthTunerSus extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        SusPensionBot robot = new SusPensionBot(hardwareMap);
        ...
        for (int i = 0; i < NUM_TRIALS; i++) {
            robot.driveRoadRunner.setPoseEstimate(new Pose2d());

            // it is important to handle heading wraparounds
            double headingAccumulator = 0;
            double lastHeading = 0;

            robot.driveRoadRunner.turnAsync(Math.toRadians(ANGLE));

            while (!isStopRequested() && robot.driveRoadRunner.isBusy()) {
                double heading = robot.driveRoadRunner.getPoseEstimate().getHeading();
            ...
            double trackWidth = DriveConstantsSus.TRACK_WIDTH * Math.toRadians(ANGLE) / headingAccumulator;
